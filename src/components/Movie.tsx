    ...
    return (
        <div className='h-dvh flex flex-col justify-center items-center'>
          <MoonLoader
            ...
            size={250}
          />
          <h1 className='bg-black p-4 text-3xl mt-6 text-yellow-500 font-staatliches'>{filmInfo && `Finding ${filmInfo.title}`}</h1>
        </div>
      )
    }
    return (
        <div className='m-4 flex bordr border-2 border-black'>
            {filmInfo && filmInfo.title && (
                <div key={filmInfo.episode_id} className='flex justify-between'>
                    ...
                    <div>
                        <h1 className='bg-black text-yellow-500 p-8 w-full text-center text-5xl uppercase font-staatliches tracking-widest font-bold'>{filmInfo.title}</h1>
